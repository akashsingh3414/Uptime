...
type UptimeStatus = "UP" | "DOWN" | "unknown";

function StatusCircle({ status }: { status: UptimeStatus }) {
  return (
    <div className={`w-3 h-3 rounded-full ${status === 'UP' ? 'bg-green-500' : status === 'DOWN' ? 'bg-red-500' : 'bg-gray-500'}`} />
  );
}
  ...
  const processedWebsites = useMemo(() => {
    return websites.map(website => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        windows[9 - i] = windowTicks.length === 0 ? "unknown" : (upTicks / windowTicks.length) >= 0.5 ? "UP" : "DOWN";
      }
